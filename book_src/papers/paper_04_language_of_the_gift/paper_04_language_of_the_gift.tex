...
% ============================================================
%  Philosophy of Intimacy and the Theory of Justice · Paper IV
%  The Gift in Intimate Relationships
%  Compile with XeLaTeX (CJK + fontspec). Series styling:
%  pale-pink page, pink §/§§ headings, bright-gold §§§/paragraph.
% ============================================================

\PassOptionsToPackage{table}{xcolor}
\usepackage{natbib}              % natbib before serendip-paper (hyperref loads inside it)
\usepackage{serendip-paper}
\usepackage{amsthm}

\newcommand{\zh}[1]{{\cjkfont #1}}

% colours used on the title page (headings now come from serendip-paper)
\definecolor{sectionpink}{RGB}{206,84,118}
\definecolor{brightgold}{RGB}{218,158,16}

% paragraph heading (not part of the shared style)
\titleformat{\paragraph}[runin]{\bfseries\color{brightgold}}{}{0em}{}
\titlespacing*{\paragraph}{0pt}{1.0ex plus .5ex}{0.8em}

\newtheorem{definition}{Definition}
\newtheorem{proposition}{Proposition}
\newtheorem{remark}{Remark}

\newcommand{\LE}{\mathcal{L}_{\mathrm{E}}}
\newcommand{\LX}{\mathcal{L}_{\mathrm{X}}}
...
\centering
\vspace*{3.2cm}
{\footnotesize\color{rosedeep}\scshape Philosophy of Intimacy and the Theory of Justice \quad$\cdot$\quad Paper IV\par}
\vspace{0.7cm}
\coverrule\par
\vspace{1.2cm}
{\color{warnred}\LARGE The Language of the Gift in Intimate Relationships: Economics, Alienation, and Generative Justice\par}
\vspace{0.9cm}
{\color{warnred}\large\itshape On the Gift as Love's Language, the Ledger It Is Read Into,\\
and the Conditions under Which the Reading Becomes Alienation\par}
\vspace{1.7cm}
\coverrule[0.25]\par
\vspace{1.0cm}
{\large Wanhong Huang\par}
\vspace{0.2cm}
{\normalsize\texttt{huangwanhong.g.official@gmail.com}\par}
\vspace{1.0cm}
{\normalsize \today\par}
\vfill
\coverfootnote
...
\zh{投我以木桃，报之以琼瑶。}\\
\zh{匪报也，永以为好也。}\\[6pt]
{\itshape You threw me a quince; I return a fine jade.\\
Not as repayment---but that our bond may last forever.}\\[4pt]
{\small from the \zh{诗经·卫风·木瓜} (Book of Songs, ``Quince'')}
\end{center}
\vspace{1.6cm}
\begin{center}
\zh{太上贵德，其次务施报。礼尚往来。}\\[6pt]
{\itshape The highest age prized virtue itself; the next attended to giving and
requiting.\\ Ritual values reciprocity.}\\[4pt]
{\small from the \zh{礼记·曲礼} (Book of Rites)}
\end{center}
\vfill
\clearpage
...
\emph{epistemic injustice} \citep{fricker2007}, since contributions outside
...
\medskip
\noindent\textbf{Keywords}\quad gift; economics of gift-giving; semiotics;
sign-value; signifier; desire; psychoanalysis; formal
language theory; interpretation; alienation; crowding out; relational work;
epistemic injustice; generative justice; recirculation; social reproduction;
...
theory of justice. Paper III \citep{huang2026vow} analyzed the unilateral
...
\begin{enumerate}[label=(\roman*)]
  \item Section~\ref{sec:econ} reviews the principal economic theories of the
  ...
  \item Section~\ref{sec:semiotic} opens the black box of expressive content.
  ...
  \item Section~\ref{sec:formal} formalizes the move with formal-language
  ...
  \item Section~\ref{sec:epistemic} shows that the kernel of $h$---the
  expressive distinctions the map collapses---is exactly where
  \citet{fricker2007}'s testimonial and hermeneutical injustices arise in the
  intimate sphere.
  \item Sections~\ref{sec:generative}--\ref{sec:data} replace the balance
  functional with a generative one (recirculation within the value-creating
  relation, after \citealp{eglash2016}) and draw design implications for
...
\citet{becker1981} models intra-family transfers through interdependent
...
\citet{akerlof1982} famously reads above-market wages and above-minimum
...
experimental work \citep{fehr1993} confirmed reciprocity as a robust
...
\citet{camerer1988} models gifts as economic signals of the giver's type or
...
\citet{prendergast2001} develop the non-monetary character of gifts as an
...
\citet{waldfogel1993} estimates that in-kind gifts destroy between a tenth
...
value'' residual. Section~\ref{sec:lacan} will return to this finding and
invert it.

\subsection{Crowding out: the gift destroyed by the price}
A fourth strand shows what happens when the two registers collide.
\citet{titmuss1970} argued that paying for blood donation degrades both the
supply and the meaning of the donated blood; \citet{gneezy2000} showed that
...
\citet{benabou2006} provide the canonical model in which extrinsic incentives
...
described}. But it lacks a general account of why; Section~\ref{sec:formal}
supplies one.

\subsection{Relational earmarking: the boundary case}
Finally, economic sociology---especially \citet{zelizer2005}---documents how
...
Section~\ref{sec:sharedmove} concluded that the economic theories are
interpretations---structure-forgetting maps from a richer system into a
poorer one. Before formalizing the map (Section~\ref{sec:formal}), we must
...
\citep{saussure1916}. Transposed to the gift: a gift signifies not by the
...
after that history. \citet{baudrillard1981} drew the economic consequence:
...
demand, and desire supplies the answer \citep{lacan2006}. A \emph{need} can
...
\citep{mauss1925}. In semiotic terms the gift is a metonym: contiguous with
...
everything established in \S\ref{sec:signvalue}--\ref{sec:lacan}---is not a
...
\citep{baudrillard1981} is, in the terms of this paper, the historical-scale
...
This section makes the ``shared move'' of Section~\ref{sec:sharedmove}
...
language. (Remark~\ref{rem:scope} states the scope limits explicitly.)
...
cross-serial dependencies in natural language \citep{shieber1985}. Nothing
...
semantics is not.} Section~\ref{sec:semiotic} supplied this clause's ground:
the value of the gift-signifier is positional (\S\ref{sec:signvalue}), and
...
Every theory in Section~\ref{sec:econ} implements some $h$: Becker's
...
this, since the failure is architectural (Definition~\ref{def:expressive}'s
...
hook for Section~\ref{sec:epistemic}.
\end{enumerate}
...
\citet{fricker2007} distinguishes \emph{testimonial} injustice (a deflated
...
apparatus of Section~\ref{sec:formal} locates both.
...
(\S\ref{def:interp}(c)), the assertion has no truth-maker in the record:
...
negotiation and legal settlement \citep{folbre2001}.
...
context-forgetting map that contracts practice (\S\ref{sec:contraction}) also
...
\citet{eglash2016} asks instead whether value is allowed to \emph{circulate
...
III of this series \citep{huang2026vow} arrived at a cognate notion from the
...
inadequacy Section~\ref{sec:formal} established. The generative alternative
...
a residual. This is the formal repair of Section~\ref{sec:epistemic}: an
...
(\S\ref{sec:contraction}); epistemic injustice is the unnameability of
$\ker h$ (\S\ref{sec:epistemic}); distributive misdescription is the
...
\S\ref{sec:contraction}---a relational instance of Goodhart's law. The
crowding-out literature (\S2.5) predicts, and Definition~\ref{def:alienation}
...
depth, and test the design program of Section~\ref{sec:data} against field
...
justice and recursive depth, on which Section~\ref{sec:generative} builds.

This series, and this paper above all, are dedicated to the one I love most
deeply---the ``forest girl'' to whom the first paper of the series was already
